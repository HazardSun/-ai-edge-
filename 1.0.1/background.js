  ...
  switch (message.action) {
    ...
    case 'detectLanguage':
      detectLanguage(message.text)
        .then(lang => sendResponse({ success: true, language: lang }))
        .catch(error => sendResponse({ success: false, error: error.message }));
      return true;
    case 'addWord':
...
async function translateText(text, targetLang) {
  ...
  let systemPrompt;
  if (customPrompt) {
    systemPrompt = customPrompt.replace(/\{targetLang\}/g, targetLangName);
  } else {
    systemPrompt = DEFAULT_PROMPT.replace(/\{targetLang\}/g, targetLangName);
  }
  ...
  if (!translation) {
    throw new Error(_t('errorEmptyResult'));
  }
  return translation;
}
...
async function detectLanguage(text) {
  ...
  const bodyText = await response.text();
  if (!response.ok)     throw new Error(_t('errorApiRequestFailed', response.status, bodyText.substring(0, 100) || ''));
  let data;
  try {
    data = JSON.parse(bodyText);
  } catch {
    throw new Error(_t('invalidJsonResponse'));
  }
