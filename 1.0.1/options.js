...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function renderWordbook(items) {
    ...
    if (filtered.length === 0) {
      ...
      if (searchTerm) {
        wordbookEmpty.querySelector('p').textContent = '没有匹配的单词';
      } else {
        wordbookEmpty.querySelector('p').textContent = '还没有收藏任何单词';
      }
      return;
    }
